    ...
    try:
        ...
        try:
            ...
            resolved_ip = socket.gethostbyname(
                target
            )  # this converts a hostname to ip (q:is this just ipv4 or both)
            return resolved_ip
...
if __name__ == "__main__":
    targetsInput = input(
        "[+] Enter target/s to scan (for multiple seperate by commas): "
    )
    portInput = input(
        "[+] Enter ports to scan (eg. 20 or 20-80 or 20,44,80 or leave blank for common ports): "
    )
        ...
        try:
            ...
            for pStr in parts:
                ...
                if 1 <= p <= 65535:
                    ...
                    print(
                        f"[-] Warning: Port {p} is out of valid range 1-65535 and will be skipped"
                    )  # 0 is reserved
            scanType = "list"
        except ValueError:
    ...
    # to handle a single port
        ...
        try:
            ...
            if 1 <= p <= 65535:
                ...
                print(
                    f"[-] Warning: Port {p} is out of valid range 1-65535 or invalid. Using common ports"
                )
                portsToScan = sorted(list(set(COMMON_PORTS)))
                scanType = "Common"
